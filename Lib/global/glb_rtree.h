#ifndef GLOBAL_RTREE_H
#define GLOBAL_RTREE_H
...
* \author: mofi. 
* \date: 2013-01-10. <br>
* 对象池，使用内存池实现一次分配多次使用，减小内存在碎片和多次申请释放内存的开销. 
*/
...
{
public:
		ObjectPool(unsigned int minSize = 1000, unsigned int maxSize=0x00ffffff)
		:m_alloctor(NULL),m_index(0),m_minSize(minSize),m_maxSize(maxSize)
		{
				if(m_minSize < 100)		m_minSize = 100;
				if(m_maxSize < 1000) 	m_maxSize = 1000;
		...
		T* alloc()
		{
				...
				if(!m_freeList.isEmpty())
						return m_freeList.takeFirst();
				// 如果分配器非空则从分配器中取
				else if(m_index < m_minSize)
						return m_alloctor[m_index++];
				// 如果创建分配器成功则从分配器中取
				else if(allocAlloctor())
						return m_alloctor[m_index++];
				// 没有对象可取
				else
						return NULL;
...
protected:
		...
		T* allocAlloctor()
		{
				if(m_total >= m_maxSize)	return NULL;
				const int count = (m_total+m_minSize >= m_maxSize ? m_maxSize-m_total: m_minSize);
				m_index = 0;
				m_alloctor = new T [count];
				m_alloctorList.append(alloctor);
				m_total += count;
				return m_alloctor;
		}
		//! 释放一个分配器
		void freeAlloctor(T* alloctor)
		{		delete[] alloctor;	}
		//! 释放所有对象
		void freeAll()
		{
				...
				m_index = 0;
				m_freeList.clear();
				const int count = m_alloctorList.size();
				for(int i=0; i<count; i++)
				{
						freeAlloctor(m_alloctorList[i]);
				}
		}
				
private:	
		std::list<T*> m_freeList;				// 自由对象列表，用于回收
		std::list<T*> m_alloctorList;		// 分配器列表，用于释放
		T*	m_alloctor;									// 当前分配器
		int m_index;										// 当前分配器索引
		
		unsigned int	m_minSize, m_maxSize, m_total;
};
#endif
...
public:
  ...
  int Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::list<DATATYPE> &result) ;
...
protected:
  ...
  ObjectPool<ListNode> m_listNodePool;
 	#endif
  ...
  bool Search(Node* a_node, Rect* a_rect, std::list<DATATYPE> &result) ;
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::list<DATATYPE> &result) 
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, std::list<DATATYPE> &result) 
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
